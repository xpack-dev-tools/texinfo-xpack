...
title: How to install the xPack GNU texinfo binaries
...
The **xPack GNU texinfo** can be installed automatically, using the `xpm` command
(the recommended method), or manually, by downloading and unpacking one of the
portable archives.

## Automated install

The easiest (and recommended) way to install GNU texinfo
is by using the **binary xPack**, available
as <a href={ `https://www.npmjs.com/package/@xpack-dev-tools/texinfo` }><code>@xpack-dev-tools/texinfo</code></a> from
...
<code>xpacks/@xpack-dev-tools/texinfo</code>.
...
**texinfo** package into the project:
...
* identify the platform specific archive for the latest available version
  of GNU texinfo, download it into a cache and unpack
...
* create a local symbolic link
  like <code>xpacks/@xpack-dev-tools/texinfo</code> pointing to the
...
* add links/forwarders into
  ...
  <code>xpacks/@xpack-dev-tools/texinfo/.content/bin</code>;
* add <code>@xpack-dev-tools/texinfo</code> to
  `package.json` as a
  development dependency; this associates a specific version of GNU texinfo with
...
it is also possible to install GNU texinfo only globally in
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/texinfo@latest --global --verbose`
} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/AppData/Roaming/xPacks/\@pack-dev-tools/texinfo /${ customField('upstreamVersion') }-${ customField('xpackSubversion') }.${ customField('npmSubversion') }/.content/bin:$PATH`
} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/Library/xPacks/\@pack-dev-tools/texinfo /${ customField('upstreamVersion') }-${ customField('xpackSubversion') }.${ customField('npmSubversion') }/.content/bin:$PATH`
} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/.local/xPacks/@xpack-dev-tools/texinfo /${ customField('upstreamVersion') }-${ customField('xpackSubversion') }.${ customField('npmSubversion') }/.content/bin:$PATH`
} </CodeBlock>
...
<CodeBlock language="sh"> {
`xpm uninstall @xpack-dev-tools/texinfo --verbose`
} </CodeBlock>

To completely remove the package from the central xPack store:

<CodeBlock language="sh"> {
`xpm uninstall --global @xpack-dev-tools/texinfo --verbose`
} </CodeBlock>
...
For all platforms, the **xPack GNU texinfo** binaries are released as portable
archives that can be installed in any location.

The archives can be downloaded from
the <a href={ `https://github.com/xpack-dev-tools/texinfo-xpack/releases/` }>GitHub Releases</a> pages.
...
The Windows versions of **xPack GNU texinfo**
are packed as `.zip` files.
Download the latest version named like:

* <code>xpack-texinfo-{ customField('upstreamVersion') }-{ customField('xpackSubversion') }-win32-x64.zip</code>
...
To manually install the <b>xPack GNU texinfo</b>,
unpack the archive and move it to a location of your choice.

The recommended location is
the <code>%USERPROFILE%\\AppData\\Roaming\\xPacks\\texinfo</code> folder, for
example <code>C:\\Users\\ilg\\AppData\\Roaming\\xPacks\\texinfo\\xpack-texinfo-{ customField('upstreamVersion') }-{ customField('xpackSubversion') }</code>.
...
* <code>xpack-texinfo-{ customField('upstreamVersion') }-{ customField('xpackSubversion') }-darwin-x64.tar.gz</code>
* <code>xpack-texinfo-{ customField('upstreamVersion') }-{ customField('xpackSubversion') }-darwin-arm64.tar.gz</code>
...
example <code>/Users/ilg/Library/xPacks/texinfo/xpack-texinfo-{ customField('upstreamVersion') }-{ customField('xpackSubversion') }</code>:

<CodeBlock language="console"> {
`mkdir -p ~/Library/xPacks/texinfo
cd ~/Library/xPacks/texinfo

tar xvf ~/Downloads/xpack-texinfo-${ customField('upstreamVersion') }-${ customField('xpackSubversion') }-darwin-x64.tar.gz
chmod -R -w xpack-texinfo-${ customField('upstreamVersion') }-${ customField('xpackSubversion') }`
} </CodeBlock>
...
example <code>/home/ilg/.local/xPacks/texinfo/xpack-texinfo-{ customField('upstreamVersion') }-{ customField('xpackSubversion') }</code>:

<CodeBlock language="console"> {
`mkdir -p ~/.local/xPacks/texinfo
cd ~/.local/xPacks/texinfo

tar xvf ~/Downloads/xpack-texinfo-${ customField('upstreamVersion') }-${ customField('xpackSubversion') }-linux-x64.tar.gz
chmod -R -w xpack-texinfo-${ customField('upstreamVersion') }-${ customField('xpackSubversion') }`
} </CodeBlock>
